...
class QuizBrain{
  ...
  QuizBrain(){
    _answer = _questionBank.length - (_questionBank.length / 4).ceil();
  }
  void nextQuestion(){
    if(_qusetionNumber < _questionBank.length - 1){
      _qusetionNumber++;
      _lastQuestion = false;
    }else{
      _lastQuestion = true;
  ...
  String getQuestionText(){
    return _questionBank[_qusetionNumber].questionText;
  }
  bool getQuestionAnswer(){
    return _questionBank[_qusetionNumber].questionAnswer;
  }
